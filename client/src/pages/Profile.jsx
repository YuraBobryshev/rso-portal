...
export default function Profile() {
  ...
  const fetchCommanderDashboard = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API_URL}/api/commander/dashboard`, { headers });
      setCommanderData(res.data);
    } catch (err) {
      console.error("Ошибка загрузки панели командира", err);
    } finally {
  ...
  const handleProcessApplication = async (appId, status) => {
    ...
    try {
      await axios.post(`${API_URL}/api/commander/process-application`, {
        appId,
        status,
    ...
    <div className="min-h-screen bg-white text-black font-sans antialiased selection:bg-rso-blue selection:text-white">
      ...
      <main className="max-w-[1500px] mx-auto px-4 sm:px-6 pt-24 pb-24 grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          {/* ВКЛАДКА 1: КАЛЕНДАРЬ МЕРОПРИЯТИЙ */}
          {profileTab === 'events' && (
                      ...
                      return (
                        <div key={event.id} onClick={() => setSelectedEventForView(event)} className="p-4 bg-gray-50/50 border border-gray-100 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 hover:bg-white hover:border-gray-200 hover:shadow-xs cursor-pointer transition-all duration-200">
                          ...
                          <div className="shrink-0 self-end sm:self-center pt-1 sm:pt-0">
                            {event.isJoined ? <span className="px-2.5 py-1 bg-green-500/10 text-green-600 rounded-lg text-[8px] sm:text-[9px] font-black uppercase tracking-wider border border-green-500/10">Вы идёте ✓</span> : <span className="px-2.5 py-1 bg-gray-100 text-gray-400 rounded-lg text-[8px] sm:text-[9px] font-black uppercase tracking-wider border border-gray-200/50">Не записан</span>}
                          </div>
                        </div>
                      );
                    })
                ...
                <div className="space-y-4 animate-fadeIn">
                  ...
                  <div className="grid grid-cols-7 gap-1 sm:gap-2">
                    {daysArray.map((day, idx) => {
                      ...
                      return (
                        <button key={idx} onClick={() => handleDayClick(day)} className={`aspect-square rounded-lg sm:rounded-xl border text-[10px] sm:text-xs font-bold flex flex-col items-center justify-center relative transition-all ${hasEvents ? 'border-blue-200 bg-blue-50/40 text-rso-blue font-black shadow-xs' : 'border-gray-50 bg-gray-50/30 text-gray-600 hover:bg-gray-100'}`}>
                          <span>{day}</span>
                        ...
                        <div key={app.id} className="p-4 bg-gray-50/50 border border-gray-100 rounded-2xl space-y-4">
                          <div className="flex justify-between items-start gap-4">
                            <div>
                              ...
                              <p className="text-xs text-gray-400 font-medium lowercase">{app.user?.email}</p>
                              <span className="text-[9px] font-mono text-gray-400 block mt-1">Подана: {new Date(app.createdAt).toLocaleDateString('ru-RU')}</span>
                            </div>
                  ...
                  <div className="space-y-3 pt-4 border-t border-gray-50">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {commanderData.members && commanderData.members.map(member => (
                        <div key={member.id} className="p-3 border border-gray-100 rounded-xl bg-white flex justify-between items-center text-xs">
                          <div className="min-w-0">
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-3 sm:p-4 animate-fadeIn">
            <div className="bg-white border border-gray-100 rounded-2xl sm:rounded-[2.5rem] w-full max-w-xl p-5 sm:p-6 md:p-8 shadow-2xl relative space-y-4 sm:space-y-6 max-h-[92vh] overflow-y-auto scrollbar-none">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[11px] sm:text-xs text-gray-700 font-medium leading-relaxed bg-gray-50/40 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-gray-100">{selectedEventForView.description || 'Организаторы не указали подробное описание для этого события.'}</p>
              </div>
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-3 border-t border-gray-100">
                <button onClick={() => setSelectedEventForView(null)} className="w-full sm:w-1/3 py-2.5 sm:py-3 border border-gray-100 text-[10px] font-black uppercase tracking-wider text-gray-400 rounded-xl hover:bg-gray-50 transition-colors order-2 sm:order-1">Закрыть</button>
                <div className="w-full sm:w-2/3 order-1 sm:order-2">
                  {selectedEventForView.isJoined ? (
                    <button onClick={() => handleLeaveEvent(selectedEventForView.id)} className="w-full py-2.5 sm:py-3 bg-red-50 text-red-600 border border-red-100 text-[10px] font-black uppercase tracking-wider rounded-xl hover:bg-red-600 hover:text-white transition-all shadow-sm">Отменить запись ×</button>
                  ) : (
                    <button onClick={() => handleJoinEvent(selectedEventForView.id)} className="w-full py-2.5 sm:py-3 bg-rso-blue text-white text-[10px] font-black uppercase tracking-wider rounded-xl hover:bg-black transition-all shadow-md shadow-blue-500/10">Я пойду →</button>
                  )}
                </div>
              </div>
            </div>
