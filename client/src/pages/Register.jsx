...
export default function Register() {
  ...
  // Обработка токена, если пользователь решил зарегаться через соцсеть
  useEffect(() => {
    ...
    if (error) {
      setServerError('Не удалось войти через социальную сеть. Попробуйте снова.');
      window.history.replaceState({}, document.title, window.location.pathname);
    }
  }, [navigate]);
  ...
  const handleRegister = async (e) => {
    ...
    if (formData.password !== formData.confirmPassword) {
      return setServerError('Пароли не совпадают');
    }
    if (formData.password.length < 6) {
      return setServerError('Пароль должен быть не менее 6 символов');
    }

    setLoading(true);

    try {
      // Отправляем данные строго по схеме бэкенда
      ...
      setServerError(err.response?.data?.message || 'Ошибка при регистрации. Проверьте данные.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50/30 dark:bg-slate-900 text-black dark:text-white font-sans flex flex-col selection:bg-rso-blue selection:text-white transition-colors duration-300">
      ...
      <main className="flex-1 flex items-start justify-center p-4 sm:p-6 pt-32 md:pt-36 pb-12">
        <div className="w-full max-w-lg bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-3xl p-6 sm:p-10 shadow-sm relative animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl font-black uppercase tracking-tight text-black dark:text-white">Регистрация</h1>
            <p className="text-xs text-gray-400 dark:text-gray-400 font-medium mt-1">Присоединяйся к крупнейшему движению Севастополя</p>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-4 mb-6">
            ...
            {/* Сетка для Имени и Фамилии */}
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1.5">Имя</label>
                <input
                  name="firstName"
                  type="text"
              ...
              <div>
                <label className="block text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1.5">Фамилия</label>
                <input
                  name="lastName"
                  type="text"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1.5">Повторите пароль</label>
                <input
          ...
          <div className="relative flex py-4 items-center">
            ...
            <span className="flex-shrink mx-4 text-[9px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Или в один клик</span>
            <div className="flex-grow border-t border-gray-100 dark:border-slate-700"></div>
          </div>
          ...
          <div className="space-y-3">
            ...
            >
              ...
              <span>Создать через ВКонтакте</span>
            </a>

            <div className="grid grid-cols-2 gap-3">
              <a 
                href="https://xn--b1af2ahcd.xn--p1ai/api/auth/google" 
                className="w-full flex items-center justify-center gap-2 border border-gray-100 dark:border-slate-700 bg-white dark:bg-slate-800 text-[10px] sm:text-xs font-bold uppercase tracking-wider text-gray-700 dark:text-gray-200 py-3.5 rounded-xl hover:bg-gray-50 dark:hover:bg-slate-700 hover:border-gray-200 dark:hover:border-slate-600 transition-all shadow-sm"
              >
                <img src="https://cdn.simpleicons.org/google" alt="Google" className="h-3.5 w-3.5" />
                <span>Google</span>
              </a>
              ...
              <a 
                ...
                className="w-full flex items-center justify-center gap-2 border border-gray-100 dark:border-slate-700 bg-white dark:bg-slate-800 text-[10px] sm:text-xs font-bold uppercase tracking-wider text-gray-700 dark:text-gray-200 py-3.5 rounded-xl hover:bg-gray-50 dark:hover:bg-slate-700 hover:border-gray-200 dark:hover:border-slate-600 transition-all shadow-sm"
              >
                <svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="#FC3F1D" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="mt-8 pt-6 border-t border-gray-50 dark:border-slate-700 text-center text-[10px] sm:text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">
            Уже есть аккаунт?{' '}
            <Link to="/login" className="text-rso-blue dark:text-blue-400 hover:text-black dark:hover:text-white transition-colors border-b border-transparent hover:border-black dark:hover:border-white pb-0.5 ml-1">
              Войти
            </Link>
          </div>
